	...
	void generateData(EntityManager em,UserTransaction ut, PrintWriter pw)
	{
		try {
			...
			for(int i=0; i < 250; ++i) {
				...
				inc.setMqMD("<mqMD<Encoding>546</Encoding><CodedCharSetId>1208</CodedCharSetId><Format>MQSTR /Format>");
				inc.setRerunsNum(0);
				inc.setServiceName("dp-service-1");
				inc.setServiceType("DP");
				inc.setStatus("NEW");
				inc.setStatusTime(new Date());
				inc.setExpiryTime(new Date(2017,10,10));
	...
	void generateOutgoing(EntityManager em,UserTransaction ut, PrintWriter pw)
	{
		try {
			...
			for(int i=0; i < 150; ++i) {
				...
				out.setIncomingId(UUID.randomUUID().toString());
				out.setMqId("1000");
				out.setMqRfh2("mqmh2");
